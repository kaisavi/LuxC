...
namespace LuxC.model
{
    class BezierCurve
    {
        ...
        public List<Point> Bezier2D(List<Point> controlPoints)
        {
            ...
            return getPointsFromp(p);
        }

        private List<Point> getPointsFromp(double[] p)
        {
        ...
        private double[] getbFromPoints(List<Point> controlPoints)
        {
            double[] b = new double[controlPoints.Count * 2];
            int bIndex = 0;

            for (int i = 0; i < controlPoints.Count; i++)
            {
                b[bIndex] = controlPoints[i].X;
                b[bIndex + 1] = controlPoints[i].Y;
                bIndex += 2;
            }
